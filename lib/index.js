...
var processAdditionalProperties = function(additionalProperties, objectName) {
  ...
  if(additionalProperties !== null) {
    forEach(additionalProperties, function(property, key) {
      ...
      props = extend(props, getSchemaProperty(modifiedProperty, key, property.required, objectName));
    });
  }
  return props;
};
...
m.getSchemas = function(definitions) {
  ...
  forEach(definitions, function(definition, key) {
    ...
    if (schemaObj && !excludedSchema) {
      ...
      schemaObj = extend(additionalProperties, schemaObj);
      var schema = new mongoose.Schema(schemaObj, options);
